...
impl TemplateAgentNode {
    ...
    /// Set MCP client (stub - MCP functionality moved to workflow-engine-mcp crate)
    pub fn with_mcp_client(self, _mcp_client_config: String) -> Self {
    ...
    async fn select_tools_from_template(
        ...
        if !self.config.template_based_tools {
            return Ok(Vec::new());
        }
        ...
        let analysis_vars = HashMap::from([
            ...
            ("available_tools".to_string(), json!(Vec::<String>::new())), // Stub - MCP tools in workflow-engine-mcp crate
        ]);
...
mod tests {
    use super::*;
    use crate::nodes::agent::ModelProvider;
