...
static mut GLOBAL_UPTIME_TRACKER: Option<UptimeTracker> = None;
...
pub fn init_uptime_tracker() -> UptimeTracker {
    INIT.call_once(|| {
        unsafe {
            GLOBAL_UPTIME_TRACKER = Some(UptimeTracker::new());
        }
    });
    
    unsafe {
        (*std::ptr::addr_of!(GLOBAL_UPTIME_TRACKER)).as_ref().unwrap().clone()
    }
}
...
pub fn get_uptime_tracker() -> Option<UptimeTracker> {
    unsafe {
        (*std::ptr::addr_of!(GLOBAL_UPTIME_TRACKER)).as_ref().cloned()
    }
}
