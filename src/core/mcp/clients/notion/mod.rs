...
impl NotionConfig {
    ...
    pub fn new_websocket(url: String) -> Self {
        Self {
            base_config: ExternalMCPConfig {
                service_name: "notion".to_string(),
                transport: crate::core::mcp::transport::TransportType::WebSocket { 
                    url, 
    ...
    pub fn new_stdio(command: String, args: Vec<String>) -> Self {
        Self {
            base_config: ExternalMCPConfig {
                service_name: "notion".to_string(),
                transport: crate::core::mcp::transport::TransportType::Stdio { 
                    command, 
                    args,
                    auto_restart: true,
                    max_restarts: 3,
...
impl NotionClientNode {
    ...
    pub async fn create_research_page(
        ...
        if !sources.is_empty() {
            ...
            for (i, source) in sources.iter().enumerate() {
                match source {
                    ...
                    Value::Object(obj) => {
                        if let (Some(title), Some(url)) = (obj.get("title"), obj.get("url")) {
                            content_parts.push(format!("{}. [{}]({})\n", i + 1, 
        ...
        if let Some(props) = properties {
            args.insert("properties".to_string(), Value::Object(props.into_iter().collect()));
        }
...
impl Node for NotionClientNode {
    fn node_name(&self) -> String {
        format!("NotionClientNode({})", self.config.base_config.service_name)
    }

    fn process(&self, mut task_context: TaskContext) -> Result<TaskContext, WorkflowError> {
        // Add Notion client info to context
        task_context.set_data(
            "notion_client_config",
            serde_json::to_value(&self.config).unwrap_or(Value::Null),
        );

        // Mark that Notion client is available
        task_context.set_data("notion_client_available", Value::Bool(true));
