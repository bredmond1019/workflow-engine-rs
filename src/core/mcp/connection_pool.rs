...
use crate::core::error::{WorkflowError, circuit_breaker::{CircuitBreaker, CircuitBreakerConfig, CircuitBreakerRegistry}};
use crate::core::mcp::clients::{MCPClient, StdioMCPClient, WebSocketMCPClient};
use crate::core::mcp::transport::TransportType;
use crate::core::mcp::health::{ConnectionHealthMonitor, HealthConfig, HealthStatus};
use crate::core::mcp::load_balancer::{MCPLoadBalancer, ConnectionInfo};
use crate::core::mcp::metrics::{MCPMetricsCollector, MCPMetricsManager};
...
impl BorrowedConnection {
    pub async fn list_tools(&self) -> Result<Vec<crate::core::mcp::protocol::ToolDefinition>, WorkflowError> {
    ...
    pub async fn call_tool(
    ...
    ) -> Result<crate::core::mcp::protocol::MCPResponse, WorkflowError> {
        let mut client = self.client.write().await;
        ...
        Ok(crate::core::mcp::protocol::MCPResponse::Result {
            id: uuid::Uuid::new_v4().to_string(),
            result: crate::core::mcp::protocol::ResponseResult::CallTool(result),
        })
    }
...
impl Drop for BorrowedConnection {
    fn drop(&mut self) {
        ...
        tokio::spawn(async move {
            pool.return_connection_internal(&server_id, &connection_id).await;
        });
    }
}
...
impl MCPConnectionPool {
    ...
    fn record_connection_request(&self, success: bool, latency: Duration) {
        ...
        if let Some(pool) = connections.get(server_id) {
            ...
            for conn in pool.iter() {
                if !conn.is_expired(self.config.idle_timeout).await {
                    let health_status = if conn.is_healthy().await {
            ...
            if let Some(selected_id) = load_balancer.select_connection(server_id, &connection_infos).await? {
                ...
                for conn in pool.iter() {
                    if conn.connection_id == selected_id && conn.is_available().await {
                        ...
                        return Ok(Some(BorrowedConnection {
                            ...
                            connection_id: conn.connection_id.clone(),
                            pool: Arc::new(self.clone()), // This needs to be fixed
                            server_id: server_id.to_string(),
                        }));
                    }
                }
                ...
                .await
            {
                Ok(client) => {
                    ...
                    let pool_client = self
                        .create_single_connection(&transport, &client_name, &client_version)
                        .await?;
                    let connection_id = self.add_to_pool(server_id, pool_client, &transport).await?;
    ...
    async fn add_to_pool(
        ...
        if pool.len() >= self.config.max_connections_per_server {
            ...
            if let Some(index) = oldest_index {
                ...
                self.health_monitor.stop_monitoring(&old_conn.connection_id).await;
            }
        }

        let connection_id = format!("{}_{}", server_id, Uuid::new_v4());
        let pooled_conn = PooledConnection::new(client, connection_id.clone(), transport_type.clone());
        pool.push(pooled_conn);

        log::debug!("Added connection {} to pool for server {}", connection_id, server_id);
        Ok(connection_id)
    }
        ...
        if let Some(pool) = connections.get(server_id) {
            for conn in pool.iter() {
                if conn.connection_id == connection_id {
                    ...
                    log::debug!("Returned connection {} to pool for server {}", connection_id, server_id);
                    break;
                }
            }
        }
    ...
    async fn create_borrowed_connection(
        ...
        let connection_id = self.add_to_pool(server_id, pool_client, &self.get_transport_for_server(server_id).await?).await?;
    ...
    async fn get_transport_for_server(&self, server_id: &str) -> Result<TransportType, WorkflowError> {
        let configs = self.server_configs.read().await;
    ...
    async fn create_single_connection_from_server(&self, server_id: &str) -> Result<Box<dyn MCPClient>, WorkflowError> {
        ...
        self.create_single_connection(&transport, &client_name, &client_version).await
    }
    ...
    pub async fn health_check(&self) -> Result<HashMap<String, bool>, WorkflowError> {
        ...
        for (server_id, pool) in connections.iter() {
            ...
            let is_healthy = healthy_count > 0 && 
                             circuit_state != crate::core::error::circuit_breaker::CircuitState::Open;
    ...
    pub async fn get_detailed_health(&self) -> DetailedHealthInfo {
        ...
        for (server_id, pool) in connections.iter() {
            ...
            server_health.insert(server_id.clone(), ServerHealthInfo {
                pool_stats,
                circuit_state,
                circuit_metrics,
            });
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                for server_id in server_configs_guard.keys() {
                    ...
                    if circuit_state != crate::core::error::circuit_breaker::CircuitState::Open {
                        let connections_guard = connections.read().await;
                        if let Some(pool) = connections_guard.get(server_id) {
                            let mut healthy_count = 0;
                            for conn in pool.iter() {
                                if *conn.is_healthy.read().await && conn.client.read().await.is_connected() {
                                    healthy_count += 1;
                                }
                            }
    ...
    pub async fn force_reconnect(&self, server_id: &str) -> Result<(), WorkflowError> {
        ...
        log::info!("Forced reconnection for server {}", server_id);
        Ok(())
    }
    ...
    pub async fn perform_health_checks(&self) -> Result<(), WorkflowError> {
        ...
        for (server_id, pool) in connections.iter() {
            for conn in pool.iter() {
                ...
                match self.health_monitor.check_connection_health(
                    &conn.connection_id,
                    &mut client_guard,
                ).await {
                    Ok(health_status) => {
                        let is_healthy = matches!(health_status, HealthStatus::Healthy | HealthStatus::Degraded);
    ...
    pub async fn recover_failed_connections(&self) -> Result<(), WorkflowError> {
        let servers_to_recover = {
            let connections = self.connections.read().await;
            let mut servers = Vec::new();
            
            for (server_id, pool) in connections.iter() {
                let healthy_count = pool.iter().filter(|conn| {
        ...
        for server_id in servers_to_recover {
            log::info!("Attempting to recover connections for server {}", server_id);
            
            // Try to create a new connection to replace failed ones
            match self.create_connection_with_retry(&server_id).await {
                Ok(_) => {
                    log::info!("Successfully recovered connection for server {}", server_id);
                }
                Err(e) => {
                    log::error!("Failed to recover connection for server {}: {}", server_id, e);
                }
            }
        }
    ...
    pub async fn update_load_balancer_weights(&self) -> Result<(), WorkflowError> {
            ...
            loop {
                ...
                if config.enable_auto_reconnect {
                    if let Err(e) = pool.recover_failed_connections().await {
                        log::error!("Connection recovery task failed: {}", e);
                    }
                }
                
                // Update load balancer weights
                if let Err(e) = pool.update_load_balancer_weights().await {
                    log::error!("Load balancer weight update failed: {}", e);
                }
                
                log::debug!("Health check task completed");
            }
        })
    }
}
...
pub struct ServerHealthInfo {
    ...
    pub circuit_state: crate::core::error::circuit_breaker::CircuitState,
...
pub struct DetailedHealthInfo {
    ...
    pub overall_summary: crate::core::mcp::health::HealthSummary,
}
