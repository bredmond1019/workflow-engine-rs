...
use crate::core::mcp::protocol::{MCPRequest, MCPResponse};
...
impl ConnectionHealthMonitor {
    ...
    pub async fn check_connection_health(
        ...
        let health_check_result = timeout(
            ...
            self.perform_health_check(client),
        ).await;
    ...
    pub async fn get_health_summary(&self) -> HealthSummary {
        ...
        let healthy_count = metrics.values().filter(|m| m.status == HealthStatus::Healthy).count();
        let degraded_count = metrics.values().filter(|m| m.status == HealthStatus::Degraded).count();
        let unhealthy_count = metrics.values().filter(|m| m.status == HealthStatus::Unhealthy).count();
        let disconnected_count = metrics.values().filter(|m| m.status == HealthStatus::Disconnected).count();
    ...
    pub async fn start_background_monitoring(&self) -> tokio::task::JoinHandle<()> {
        ...
        tokio::spawn(async move {
            ...
            while *running.read().await {
                ...
                log::debug!("Background health monitoring tick");
...
mod tests {
    ...
    async fn test_health_summary() {
        ...
        monitor.start_monitoring("conn1".to_string(), "server1".to_string()).await;
        monitor.start_monitoring("conn2".to_string(), "server2".to_string()).await;
