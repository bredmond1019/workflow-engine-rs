...
use crate::core::mcp::connection_pool::{PoolStats, LoadBalancingStrategy};
use crate::core::mcp::health::HealthStatus;
...
impl MCPLoadBalancer {
    ...
    pub async fn select_connection(
        ...
        let connection_id = match self.strategy {
            LoadBalancingStrategy::RoundRobin => {
                self.select_round_robin(server_id, available_connections).await
            }
            LoadBalancingStrategy::Random => {
                self.select_random(available_connections).await
            }
    ...
    async fn select_health_based(&self, connections: &[ConnectionInfo]) -> Option<String> {
        ...
        weighted_connections.last().map(|(conn, _)| conn.connection_id.clone())
    }
...
impl AdvancedMCPLoadBalancer {
    ...
    pub async fn select_connection_advanced(
    ...
    ) -> Result<Option<(String, String)>, WorkflowError> { // Returns (server_id, connection_id)
        ...
        if let Some(client_id) = client_id {
            ...
            if let Some(preferred_server) = affinities.get(client_id) {
                if let Some(connections) = available_servers.get(preferred_server) {
                    if let Some(conn_id) = self.balancer
        ...
        for (server_id, connections) in available_servers {
            ...
            let health_score = connections.iter()
            ...
            let load_score = 1.0 - (connections.iter().map(|c| c.use_count).sum::<u64>() as f64 / 
    ...
    async fn record_request(&self, server_id: &str) {
        ...
        *metrics.requests_per_server.entry(server_id.to_string()).or_insert(0) += 1;
    }
...
mod tests {
    ...
    async fn test_round_robin_selection() {
        ...
        let first = balancer.select_connection("server1", &connections).await.unwrap();
        let second = balancer.select_connection("server1", &connections).await.unwrap();
        let third = balancer.select_connection("server1", &connections).await.unwrap();
        let fourth = balancer.select_connection("server1", &connections).await.unwrap();
    ...
    async fn test_least_connections_selection() {
        ...
        let selected = balancer.select_connection("server1", &connections).await.unwrap();
    ...
    async fn test_health_based_selection() {
        ...
        let connections = vec![
            ...
            create_test_connection("conn2", "server1", HealthStatus::Healthy, 0, Some(Duration::from_millis(100))),
            create_test_connection("conn3", "server1", HealthStatus::Unhealthy, 0, Some(Duration::from_millis(300))),
        ];
        ...
        for _ in 0..10 {
            if let Some(selected) = balancer.select_connection("server1", &connections).await.unwrap() {
                if selected == "conn2" {
                    healthy_selected += 1;
                }
            }
    ...
    async fn test_advanced_load_balancer() {
        ...
        // Set client affinity
        balancer.set_client_affinity("client1".to_string(), "server1".to_string()).await;
        
        let mut available_servers = HashMap::new();
        available_servers.insert("server1".to_string(), vec![
            create_test_connection("conn1", "server1", HealthStatus::Healthy, 0, None),
        ]);
        available_servers.insert("server2".to_string(), vec![
            create_test_connection("conn2", "server2", HealthStatus::Healthy, 0, None),
        ]);

        // Test affinity selection
        let result = balancer.select_connection_advanced(
            Some("client1"), 
            &available_servers
        ).await.unwrap();
