...
impl Workflow {
    ...
    /// Runs the workflow from an existing Event.
    ///
    /// # Arguments
    ///
    /// * `event` - A reference to the `Event` to run the workflow from.
    ///
    /// # Returns
    ///
    ...
    ///
    /// ```
    /// use your_crate::{Workflow, Event};
    ///
    /// let workflow = Workflow::new(schema).expect("Failed to create workflow");
    /// let event = Event::new("example_workflow", serde_json::json!({}));
    /// let result = workflow.run_from_event(&event);
    /// ```
    pub fn run_from_event(&self, event: &Event) -> Result<TaskContext, WorkflowError> {
        // Validate workflow type matches
        if event.workflow_type != self.schema.workflow_type {
            return Err(WorkflowError::WorkflowTypeMismatch {
                expected: self.schema.workflow_type.clone(),
                actual: event.workflow_type.clone(),
            });
        }

        let mut task_context = TaskContext::from_event(event);
    ...
    /// Exposes the workflow nodes as an MCP server
    ///
    /// # Arguments
    ///
    ...
    /// # Returns
    ///
    /// Returns a `Result` containing the `MCPToolServer` if successful,
    /// or a `WorkflowError` if setup fails.
    ///
    /// # Examples
    ///
    /// ```
    ...
    ///
    /// let workflow = Workflow::new(schema).expect("Failed to create workflow");
    /// let mcp_server = workflow.expose_as_mcp_server("workflow-server", "1.0.0").await;
    /// ```
    pub async fn expose_as_mcp_server(
        &self,
        server_name: &str,
        server_version: &str,
    ) -> Result<MCPToolServer, WorkflowError> {
        let mcp_server = MCPToolServer::new(server_name.to_string(), server_version.to_string());

        // Register all nodes in the registry as MCP tools
        {
            let registry = self.registry.read().unwrap();
            let node_types = registry.get_all_node_types();

            for node_type in node_types {
                if let Some(node) = registry.get(&node_type) {
                    let node_wrapper = NodeWrapper::new(node, node_type, self.registry.clone());
                    let node_arc = Arc::new(node_wrapper);
                    mcp_server
                        .register_node_with_auto_metadata(node_arc)
                        .await?;
                }
            }
        }
    ...
    /// Registers an external MCP server for use by agent nodes
    ///
    /// # Arguments
    ///
    /// * `server_uri` - The URI of the external MCP server
    /// * `transport_type` - The transport type to use for connection
    ///
    /// # Returns
    ///
    ...
    ///
    /// ```
    /// use your_crate::{Workflow, TransportType};
    ///
    /// let workflow = Workflow::new(schema).expect("Failed to create workflow");
    /// workflow.register_mcp_server("stdio:///path/to/server", TransportType::Stdio { command: "server".to_string(), args: vec![] }).await;
    /// ```
    pub async fn register_mcp_server(
        &self,
        server_uri: &str,
        transport_type: crate::core::mcp::transport::TransportType,
    ) -> Result<(), WorkflowError> {
        // This method would be used to register external MCP servers
        // that agent nodes can connect to. Implementation would depend on
        // how you want to manage the connection pool/registry.

        // For now, this is a placeholder that could store server configurations
        // in the workflow for later use by agent nodes
        log::info!(
            "Registering MCP server: {} with transport: {:?}",
            server_uri,
            transport_type
        );
        Ok(())
